 ...
 * The default is to use a cookie + {@link https://nxauth.khulnasoft.com/concepts/session-strategies#jwt-session JWT}
 * based session store (`strategy: "jwt"`),
 * but you can also use a database adapter to store the session in a database.
 *
 * Before you continue, Auth.js has a list of {@link https://adapters.nxauth.khulnasoft.com official database adapters}. If your database is listed there, you
 ...
 *
 * :::caution Note
 * Although `@nxauth/core` _is_ framework/runtime agnostic, an adapter might rely on a client/ORM package,
 ...
 *
 * ```bash npm2yarn
 * npm install @nxauth/core
 * ```
 *
 * Then, you can import this submodule from `@nxauth/core/adapters`.
 *
 * ## Usage
 *
 ...
 * import { type Adapter } from "@nxauth/core/adapters"
 *
 ...
 * ## Testing
 *
 * There is a test suite [available](https://github.com/khulnasoft/nxauth/blob/main/packages/utils/adapter.ts)
 ...
 * ### Token rotation
 *
 * Auth.js _currently_ does not support {@link https://nxauth.khulnasoft.com/concepts/oauth `access_token` rotation} out of the box.
 ...
 * in the core library.
 * [This guide](https://nxauth.khulnasoft.com/guides/refresh-token-rotation#database-strategy) should provide the necessary steps to do this in user land.
 *
 * ### Federated logout
 *
...
export interface AdapterUser extends User {
  ...
  email: string
  /**
   * Whether the user has verified their email address via an [Email provider](https://nxauth.khulnasoft.com/getting-started/authentication/email).
 ...
 * - Email accounts, which are created when a user signs in with an [Email provider](https://nxauth.khulnasoft.com/getting-started/authentication/email).
 *
 ...
 * It is created when a user signs in with an [Email provider](https://nxauth.khulnasoft.com/getting-started/authentication/email).
...
export interface Adapter {
   ...
   *
   * See also [User management](https://nxauth.khulnasoft.com/guides/creating-a-database-adapter#user-management)
   */
   ...
   *
   * See also [Verification tokens](https://nxauth.khulnasoft.com/guides/creating-a-database-adapter#verification-tokens)
   */
   ...
   * It creates an [Account](https://nxauth.khulnasoft.com/reference/core/adapters#models) in the database.
   *
   * See also [User management](https://nxauth.khulnasoft.com/guides/creating-a-database-adapter#user-management)
   */
  linkAccount?(
   ...
   *
   * See also [Database Session management](https://nxauth.khulnasoft.com/guides/creating-a-database-adapter#database-session-management)
   */
...
declare module "nxauth/adapters" {
  type JsonObject = {
