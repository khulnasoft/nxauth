 ...
 *
 * For use in [Server Actions](https://nextjs.org/docs/app/api-reference/functions/server-actions), check out [these methods](https://nxauth.khulnasoft.com/guides/upgrade-to-v5#methods)
 *
 * @module react
 */
...
import type { LoggerInstance, Session } from "@nxauth/core/types"
import type {
  AuthClientConfig,
  ClientSafeProvider,
...
function getNewBroadcastChannel() {
  return new BroadcastChannel("nxauth")
}
...
export function useSession<R extends boolean>(
  ...
  if (!value && process.env.NODE_ENV !== "production") {
    throw new Error(
      "[nxauth]: `useSession` must be wrapped in a <SessionProvider />"
    )
  }
  ...
  if (providerType === "webauthn") {
    ...
    throw new TypeError(
      [
        ...
        'Please use `import { signIn } from "nxauth/webauthn"` instead.',
      ].join("\n")
    )
  }
