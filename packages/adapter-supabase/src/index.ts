 ...
 *   <img style={{display: "block"}} src="https://nxauth.khulnasoft.com/img/adapters/supabase.svg" width="50"/>
 *  </a>
 * </div>
 *
 * ## Installation
 *
 * ```bash npm2yarn
 * npm install @supabase/supabase-js @nxauth/supabase-adapter
 * ```
 *
 * @module @nxauth/supabase-adapter
 */
...
export function SupabaseAdapter(options: SupabaseAdapterOptions): Adapter {
  ...
  const supabase = createClient<Database, "next_auth">(url, secret, {
    db: { schema: "next_auth" },
    global: { headers: { "X-Client-Info": "@nxauth/supabase-adapter" } },
