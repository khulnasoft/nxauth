 ...
 *
 * :::warning
 * `@nxauth/express` is currently experimental. The API _will_ change in the future.
 * :::
 *
 ...
 * The callback URL used by the [providers](https://nxauth.khulnasoft.com/reference/core/providers) must be set to the following, unless you mount the `ExpressAuth` handler on a different path:
 *
 * ```
 ...
 * Once your application is mounted you can sign in or out by making requests to the following [REST API endpoints](https://nxauth.khulnasoft.com/reference/core/types#authaction) from your client-side code.
 ...
 *
 * ```ts title="app.ts"
 * import { getSession } from "@nxauth/express"
 *
 ...
 *
 * ## Notes on ESM
 * @nxauth/express is ESM only. This means your package.json must contain `"type": "module"` and tsconfig.json should contain `"module": "NodeNext"` or `ESNext`.
 ...
 *
 * While it is NOT recommended, if you wish to use @nxauth/express within a CommonJS project without migrating and making the above changes, you can run the dev server with tsx and may be able to compile with [pkgroll](https://tsx.is/compilation).
...
import type { Session } from "@nxauth/core/types"
