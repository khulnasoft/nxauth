/**
 * :::warning
 * `@nxauth/sveltekit` is currently experimental. The API _might_ change.
 * :::
 *
 ...
 *
 * ### Lazy initialization
 * `@nxauth/sveltekit` supports lazy initialization where you can read the `event` object to lazily set the configuration. This is especially useful when you have to get the environment variables from `event.platform` for platforms like Cloudflare Workers.
 *
 * ```ts title="src/auth.ts"
 * import { SvelteKitAuth } from "@nxauth/sveltekit"
 * import GitHub from "@nxauth/sveltekit/providers/github"
 ...
 *
 * The callback URL used by the [providers](https://nxauth.khulnasoft.com/getting-started/providers) must be set to the following, unless you override {@link SvelteKitAuthConfig.basePath}:
 *
 * ```
 ...
 * ### Server-side
 *
 * `<SignIn />` and `<SignOut />` are components that `@nxauth/sveltekit` provides out of the box - they handle the sign-in/signout flow, and can be used as-is as a starting point or customized for your own components. This is an example of how to use the `SignIn` and `SignOut` components to login and logout using SvelteKit's server-side form actions. Another example is available on [our svelte-auth-example repo](https://github.com/khulnasoft/sveltekit-auth-example).
 *
 ...
 * <script>
 *   import { SignIn, SignOut } from "@nxauth/sveltekit/components"
 ...
 * ### Client-Side
 *
 * We also export two methods from `@nxauth/sveltekit/client` in order to do client-side sign-in and sign-out actions.
 *
 * ```ts title="src/routes/+page.svelte"
 * <script>
 *   import { signIn, signOut } from "@nxauth/sveltekit/client"
 *   let password
 * </script>
 *
 * <nav>
 ...
 *     <code>@nxauth/sveltekit/client</code>
 *   </p>
 *   <div class="actions">
 ...
 * ## Notes
 *
 * - If you build your SvelteKit application with `prerender` enabled, pages which have an anchor tag to the default signin page (i.e. `<a href="/auth/signin" ...`) will have trouble building. Please use the [builtin functions or components](https://nxauth.khulnasoft.com/getting-started/session-management/login?framework=sveltekit) to sign in or out instead.
 *
 * :::info
 * Learn more about `@nxauth/sveltekit` [here](https://vercel.com/blog/announcing-sveltekit-auth).
 * :::
 *
 * @module @nxauth/sveltekit
 */
...
import { Auth, isAuthAction, customFetch } from "@nxauth/core"
import { building } from "$app/environment"

export { customFetch }
export { AuthError, CredentialsSignin } from "@nxauth/core/errors"
...
/**
 * The main entry point to `@nxauth/sveltekit`
 * @see https://sveltekit.nxauth.khulnasoft.com
 */
