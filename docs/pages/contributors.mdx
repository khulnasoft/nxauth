...
You can find us on [Open Collective](https://opencollective.com/khulnasoft). We are very thankful for all of our existing contributors and would be delighted if you or your company would decide to join them.
...
Thank you to the [dozens of individual contributors](https://github.com/khulnasoft/nxauth/graphs/contributors) who have helped shape Auth.js.
...
Database Adapters were moved to their packages under the name `@nxauth/*-adapter`.

### 2022 – Birth of Auth.js

Based on NextAuth.js, Balázs released [Auth.js (`@nxauth/core`)](https://twitter.com/balazsorban44/status/1603082914362986496), a runtime/framework independent core library that is the base of
...
Balázs returned and continued the work on Auth.js. As a pilot project, `nxauth@experimental` (later `nxauth@beta`) releases were published
...
The new default documentation page became [nxauth.khulnasoft.com](https://nxauth.khulnasoft.com) (the one you are reading right now), and the old NextAuth.js documentation at [nxauth.js.org](https://nxauth.js.org) was kept around
to document NextAuth.js v4 and is kept only as a back reference.

Database Adapters were moved from the `@nxauth/*-adapter` namespace to `@nxauth/*-adapter`, indicating that they are not NextAuth.js specific anymore.
...
All official integrations are distributed under the `@nxauth` scope except for NextAuth.js, which is distributed under `nxauth` to reduce migration overhead.
