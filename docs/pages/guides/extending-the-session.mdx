...
```ts filename="auth.ts"
  //  By default, the `id` property does not exist on `token` or `session`. See the [TypeScript](https://nxauth.khulnasoft.com/getting-started/typescript) on how to add it.
  callbacks: {
    jwt({ token, user }) {
...
```ts filename="auth.ts"
  //  By default, the `id` property does not exist on `session`. See the [TypeScript](https://nxauth.khulnasoft.com/getting-started/typescript) on how to add it.
  callbacks: {
...
import type { Provider } from "nxauth/providers"
