...
Auth.js adapters allow you to integrate with any (even multiple) database/back-end service, even if we don't have an [official package](https://github.com/khulnasoft/nxauth/tree/main/packages) available yet. (We welcome PRs for new adapters! See the [guidelines](#official-adapter-guidelines) below.)
...
Optionally, you can run our [Adapter tests](https://github.com/khulnasoft/nxauth/blob/main/packages/utils/adapter.ts) on your adapter to ensure it is compliant with the Auth.js.
...
1. Configure the monorepo to help us maintain the package
   ...
   - Add a (preferably `.svg`) logo to [this directory](https://github.com/khulnasoft/nxauth/tree/main/docs/public/img/adapters)
   - Add the Adapter to our GitHub workflow files [here](https://github.com/khulnasoft/nxauth/tree/main/.github/workflows/release.yml#L12) and [here](https://github.com/khulnasoft/nxauth/tree/main/.github/pr-labeler.yml)
   - Make sure to [`.gitignore` generated files](https://github.com/khulnasoft/nxauth/tree/main/.gitignore#L58) if there are any
...
You can take advantage of the types that comes with the framework packages (i.e. `nxauth/adapters`, `@nxauth/sveltekit/adapters`).

```ts
import type { Adapter } from "nxauth/adapters"
...
```js {1}
/** @return { import("nxauth/adapters").Adapter } */
function MyAdapter() {
  return {
    // your adapter methods here
  }
...
- [Official adapters' source code](https://github.com/khulnasoft/nxauth/tree/main/packages)
