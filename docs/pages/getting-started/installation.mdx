...
<Callout type="info">
  Installing `@nxauth/core` is not necessary, as a user you should never have to
  interact with `@nxauth/core`.
</Callout>
...
verification hashes. (See [Deployment](/getting-started/deployment) to learn more). You can generate one via the official [Auth.js CLI](https://cli.nxauth.khulnasoft.com) running:
...
1. Start by creating a new `plugin@nxauth.ts` file in your `src/routes` directory with the following content.
