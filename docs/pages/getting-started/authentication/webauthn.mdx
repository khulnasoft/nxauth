...
- `nxauth@5.0.0-beta.8` or above
- `@nxauth/prisma-adapter@1.3.0` or above
...
This is the raw SQL migration for PostgreSQL, for more details including example migrations for other databases, check out the updated Prisma schemas at the [`@nxauth/prisma-adapter` docs](/getting-started/adapters/prisma).
...
If you're using a custom signin page, you can leverage the `nxauth` `signIn` function to initiate WebAuthn registration and login flows with the following code.
