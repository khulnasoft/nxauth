...
<Callout type="info">
  This guide only applies to `nxauth` upgrades for users of Next.js. Feel free
  to skip to the next section, [Installation](/getting-started/installation), if
  you are not upgrading to `nxauth@5`.
</Callout>

**NextAuth.js version 5** is a major rewrite of the `nxauth` package, that being said, we introduced as few breaking changes as possible. For all else, this document will guide you through the migration process.

Get started by installing the latest version of `nxauth` with the `beta` tag.
...
- Auth.js now builds on `@nxauth/core` with stricter [OAuth](https://www.ietf.org/rfc/rfc6749.html)/[OIDC](https://openid.net/specs/openid-connect-core-1_0.html) spec-compliance, which might break some existing OAuth providers. See our [open issues](https://github.com/khulnasoft/nxauth/issues?q=is%3Aopen+is%3Aissue+label%3Aproviders) for more details.
- OAuth 1.0 support is deprecated.
- The minimum required Next.js version is now [14.0](https://nextjs.org/14).
- The import `nxauth/next` is replaced. See [Authenticating server-side](#authenticating-server-side) for more details.
- The import `nxauth/middleware` is replaced. See [Authenticating server-side](#authenticating-server-side) for more details.
- When the `idToken: boolean` option is set to `false`, it won't entirely disable the ID token. Instead, it signals `nxauth` to also visit the `userinfo_endpoint` for the final user data. Previously, `idToken: false` opted out to check the `id_token` validity at all.
...
2. There is no need to install `@nxauth/core` to import the provider definitions from, these come from `nxauth` itself.
...
- import { getServerSession } from "nxauth/next"
...
- export { default } from "nxauth/middleware"
...
Instead of importing `getServerSession` from `nxauth/next` or `getToken` from `nxauth/jwt`, you can now import the `auth` function exported from your `auth.ts` config file and call it without passing `authOptions`.

```diff filename='pages/api/example.ts'
- import { getServerSession } from "nxauth/next"
- import { getToken } from "nxauth/jwt"
- import { authOptions } from "pages/api/auth/[...nextauth]"
+ import { auth } from "@/auth"
...
Instead of importing `getServerSession` from `nxauth/next` or `getToken` from `nxauth/jwt`, you can now import the `auth` function from your config file and call it without passing `authOptions`.
...
Beginning with `nxauth` v5, you should now install database adapters from the `@nxauth/*-adapter` scope, instead of `@nxauth/*-adapter`. Database adapters don't rely on any Next.js features, so it made more sense to move them to this new scope.

```diff
- npm install @nxauth/prisma-adapter
+ npm install @nxauth/prisma-adapter
```
...
- The following types are now exported from all framework packages like `nxauth` and `@nxauth/sveltekit`:
...
- All `Adapter` types are re-exported from `/adapters` in the framework packages as well, i.e. from `nxauth/adapters`, `@nxauth/sveltekit/adapters`, etc.

</Steps>

## Cookies

- The `nxauth` prefix is renamed to `authjs`.

## Summary

We hope this migration goes smoothly for everyone! If you have any questions or get stuck anywhere, feel free to create [a new issue](https://github.com/khulnasoft/nxauth/issues/new) on GitHub, or come chat with us in the [Discord](https://discord.nxauth.khulnasoft.com) server.
