...
Auth.js libraries come with certain interfaces that are shared across submodules and different Auth.js libraries (For example: `nxauth` and `@nxauth/prisma-adapter` will rely on types from `@nxauth/core`).
...
```ts filename="auth.ts"
import SvelteKitAuth, { type DefaultSession } from "@nxauth/sveltekit"

declare module "@nxauth/sveltekit" {
  interface Session {
    user: {
      userId: string
      /**
...
// The `JWT` interface can be found in the `nxauth/jwt` submodule
import { JWT } from "nxauth/jwt"
