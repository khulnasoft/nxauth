...
```bash npm2yarn
npm install @nxauth/dynamodb-adapter @aws-sdk/lib-dynamodb @aws-sdk/client-dynamodb
```
...
The default table name is `nxauth`, but you can customise that by passing `{ tableName: 'your-table-name' }` as the second parameter in the adapter.
...
import { DynamoDBAdapter } from "@nxauth/dynamodb-adapter"
