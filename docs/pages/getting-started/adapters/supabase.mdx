...
import { SupabaseAdapter } from "@nxauth/supabase-adapter"
...
Setup your database as described in our main [schema](https://nxauth.khulnasoft.com/reference/core/adapters#models), by copying the SQL schema below in the Supabase [SQL Editor](https://app.supabase.com/project/_/sql).
...
// https://nxauth.khulnasoft.com/reference/core/types#authconfig
export const { handlers, auth, signIn, signOut } = NextAuth({
...
In order to extend the `session` object with the `supabaseAccessToken` we need to extend the `session` interface in a `types/nxauth.d.ts` file:

```ts filename="types/nxauth.d.ts"
import NextAuth, { type DefaultSession } from "nxauth"
