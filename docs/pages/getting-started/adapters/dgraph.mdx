...
```bash npm2yarn
npm install @nxauth/dgraph-adapter
```
...
import { DgraphAdapter } from "@nxauth/dgraph-adapter"
...
> This schema is adapted for use in Dgraph and based upon our main [schema](https://nxauth.khulnasoft.com/reference/core/adapters)
...
by nxauth. The main form of access control used in Dgraph is via `@nxauth` directive alongside types in the schema.
...
secure interactions to be made with all the auth types required by nxauth. You have to specify it for each auth rule of
...
### JWT session and `@nxauth` directive

Dgraph only works with HS256 or RS256 algorithms. If you want to use session jwt to securely interact with your dgraph
database you must customize nxauth `encode` and `decode` functions, as the default algorithm is HS512. You can
...
Once your `Dgraph.Authorization` is defined in your schema and the JWT settings are set, this will allow you to define [`@nxauth rules`](https://dgraph.io/docs/graphql/authorization/authorization-overview/) for every part of your schema.
