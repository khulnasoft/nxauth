...
test("Sign in with Auth0", async ({ page }) => {
  ...
  await page.goto("https://nxauth-example.vercel.app")
  ...
  // Wait for nxauth example page login status header to appear
  await page.waitForTimeout(2000)

  // Snap a screenshot
  // await page.screenshot({
  //   path: "2-nxauth-redirect-result.png",
  //   fullPage: false,
  // })

  // Check session object after successful login
  const response = await page.goto(
    "https://nxauth-example.vercel.app/api/auth/session"
  )
